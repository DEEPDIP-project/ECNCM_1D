...
function gen_D1(x)
    dx = x[2] - x[1]
    N = size(x)[1]
    return 1 / (2 * dx) * gen_stencil(N, [-1.0, 0.0, 1.0], [-1, 0, 1])
end

function gen_D2(x)
    dx = x[2] - x[1]
    N = size(x)[1]
    return 1 / (dx^2) * gen_stencil(N, [1.0, -2.0, 1.0], [-1, 0, 1])
end

function gen_D3(x)
    dx = x[2] - x[1]
    N = size(x)[1]
    return 1 / (2 * dx^3) * gen_stencil(N, [-1.0, 2.0, -2.0, 1.0], [-2, -1, 1, 2])
end
...
function gen_fourier(
    ...
    if min_fourier_mode == 0 && max_fourier_mode == 0
        ...
        coeffs =
            sign.(rand(Uniform(-1.0, 1.0), 2 * max_mode)) .*
            rand(Uniform(0.5, 1), 2 * max_mode)
    ...
    function funct(
        ...
        eval =
            scaling / sqrt(max_mode) * sum(
                [
                    coeffs[i] * sin.(i * frequency * x) .+
                    coeffs[i+max_mode] * cos.(i * frequency * x) for i = 1:max_mode
                ],
                dims = 1,
            )[1]
        eval = eval .+ offset
...
function process_HR_solution(us, dus, ts, domain_descriptors, f, return_primes = false)
    ...
    return Dict([
        ...
        ("HR_u_bar", HR_us_bar),
        ("u_prime", HR_us_prime),
        ("HR_du_bar", HR_dus_bar),
        ("du_prime", HR_dus_prime),
        ("dE", dEs),
        ("dE_bar", dEs_bar),
        ("dE_prime", dEs_prime),
    ])
...
function gen_conv_burgers_dudt(viscosity)
    conv_D1 = gen_conv_stencil([-1.0, 0.0, 1.0])
    conv_D2 = gen_conv_stencil([1.0, -2.0, 1.0])
...
function gen_conv_KdV_dudt()
    conv_D1 = gen_conv_stencil([-1.0, 0.0, 1.0])
    conv_D3 = gen_conv_stencil([-1.0, 2.0, 0.0, -2.0, 1.0])
...
function gen_rand_condition(
    ...
    if in_outflow
        ...
        F =
            0.5 *
            gen_fourier(domain_range, min_mode, max_mode, offset = 0, scaling = scaling)(
                ref_x,
            )
