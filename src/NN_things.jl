...
function enforce_momentum_conservation(stencil, symm = "none", mom_cons = true)
    ...
    if symm == "skew-symm"
        bar = [
            [stencil[i] for i = 1:stencil_neighbors]
            [0]
            reverse([-stencil[i] for i = 1:stencil_neighbors])
        ]
    elseif symm == "symm"
        bar = [
            [stencil[i] for i = 1:(stencil_neighbors+1)]
            reverse([stencil[i] for i = 1:stencil_neighbors])
        ]
        if mom_cons
            bar = bar .- mean(bar)
...
function gen_select_mat(x, neighbors, reduction_fold = 1)
    ...
    mat = zeros(Int, (J, K))
    for j = 1:J
        for k = -neighbors:neighbors
            for l = 1:reduction_fold
                mat[j, reduction_fold*(k+neighbors)+1+(l-1)] = index_converter(
                    reduction_fold * (j + k) + (l - reduction_fold),
                    reduction_fold * J,
                )
            end
        end
...
function init_stencil(stencil_width, coeff = 0.4)
    stencil =
        exp.(-coeff * collect(-stencil_width:stencil_width) .^ 2) .*
        (-ones(2 * stencil_width + 1)) .^ collect(1:2*stencil_width+1)
...
function gen_model(f, constraints, supply_s, dissipation, NN_descriptors; stencils = 0)
    ...
    model(
        ...
        pad_sizes = (physics_width, stencil_width, conv_pad_size),
        supply_s = supply_s,
        diagonals = diagonals;
        channel = "all",
    ) = skew_symm_NN(
...
function skew_symm_NN(
    ...
    pad2 = BC_f(
        ...
        bs = bs,
        eval_BCs = false,
        pad_size = conv_pad_size + physics_width,
    )
    ...
    if constraints
        ...
        pad_s = padding(
            s,
            2 * stencil_width + diagonals,
            0 * as,
            0 * bs,
            anti_symm_outflow = true,
        )
        ...
        ex_s =
            -exchange_form(reverse(B32), reverse(Phi3), reverse(B31_bar), pad_u, diagonals)
        ...
        if dissipation
            ...
            Psi2 = Tuple(
                p[:, i, :] for i in select_vec[1:end-diagonals] .+ 4 * add .- 1 * diagonals
            )

            Psi3 = Tuple(
                p[:, i, :] for i in select_vec[1:end-diagonals] .+ 5 * add .- 2 * diagonals
            )
            Psi4 = Tuple(
                p[:, i, :] for i in select_vec[1:end-diagonals] .+ 6 * add .- 3 * diagonals
            )
            ...
            diss_u =
                new_diss_form(B1_bar, Psi1, Psi1, B1_bar, pad_u, diagonals) .+
                new_diss_form(B3_bar, Psi3, Psi3, B3_bar, pad_u, diagonals) .+
                new_diss_form(B1_bar, Psi1, Psi2, B2, pad_s, diagonals) .+
                new_diss_form(B3_bar, Psi3, Psi4, B4, pad_s, diagonals)
            diss_s =
                new_diss_form(B2, Psi2, Psi1, B1_bar, pad_u, diagonals) .+
                new_diss_form(B4, Psi4, Psi3, B3_bar, pad_u, diagonals) .+
                new_diss_form(B2, Psi2, Psi2, B2, pad_s, diagonals) .+
                new_diss_form(B4, Psi4, Psi4, B4, pad_s, diagonals)
...
function trajectory_loss_function(
    us,
    ss,
    as,
    bs,
    ...
    for i = 1:size(vec)[2]
        ...
        elseif isnan(a[i]) && isnan(b[i]) == false
            if anti_symm_outflow
                ...
                back = [back (
                    2 * vec[end, i] .- 2 * (vec[end, i] - b[i]) .-
                    reverse(vec[end-pad_size+1:end, i])
                )]
                front = [front reverse(vec[1:pad_size, i])]
            end
        elseif isnan(b[i]) && isnan(a[i]) == false
            ...
            if anti_symm_outflow
                ...
                back = [back reverse(vec[end-pad_size+1:end, i])]
                front = [front (
                    (2 * vec[1, i]) .- 2 * (vec[1, i] - a[i]) .-
                    reverse(vec[1:pad_size, i])
                )]
            end
        else
            #front = [ front a[i]*vec[end-pad_size+1:end,i].^0]
...
function NeuralNetwork(layers; bias = true, activation_function = relu)
    ...
    for i = 1:size(layers)[1]-1
        if i == size(layers)[1] - 1
            ...
            storage =
                [storage; Dense(layers[i], layers[i+1], activation_function, bias = bias)]
        end
    end
...
function gen_subgrid_filter(domain_descriptors, outflow = false)
    ...
    function subgrid_filter(
        ...
        filtered =
            vcat([t_vec' * u_primes[select_mat[:, i], :] for i = 1:size(select_mat)[2]]...)
        return filtered
    end
...
function gen_train_test_set(d, f, domain_descriptors, fraction, train_fraction)
    ...
    indexes =
        cut_indexes(us[:, 1:ending_index], fraction, randomize = false, uniform = false)

    data = Dict()
    test_data = Dict()

    BC_f = padding_wrapper(f, eval_BCs = false, as = as[:, indexes], bs = bs[:, indexes])
    data = process_HR_solution(
        us[:, indexes],
        dus[:, indexes],
        ts[:, indexes],
        domain_descriptors,
    ...
    test_BC_f = padding_wrapper(
        f,
        eval_BCs = false,
        as = as[:, test_indexes],
        bs = bs[:, test_indexes],
    )
    test_data = process_HR_solution(
        us[:, test_indexes],
        dus[:, test_indexes],
        ts[:, test_indexes],
        domain_descriptors,
...
function gen_trajectory_data(
    ...
    trajectory_data["s"] =
        reshape(T_mat * traj_data["u_prime"], (I, traj_steps + 1, data_size))
...
function import_model(path, f)
    path = process_path(path)
    constraints, supply_s, dissipation, NN_descriptors, stencils, weight_bias = (
        load(path * "model.jld")[i] for i in (
            "constraints",
            "supply_s",
            "dissipation",
            "NN_descriptors",
